...
describe('users e2e', () => {
    ...
    it('Info - sucsess', async () => {
        const login = await request(application.app).post('/users/login').send({ email: 'makak@gmail.com', password: '132' })
        const res = await request(application.app).get('/users/info').set('Authorization', `Bearer ${login}`)
        expect(res.statusCode).toBe(401)
    })
})
